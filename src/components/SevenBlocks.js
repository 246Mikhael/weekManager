...
function SevenBlocks({numberOfWeek, increase, decrease, currentWeek}){
  function id(){
    return nanoid();
  }
  const days = [
   ...
   let today = createDate(getCurrentDate(0));
     ...
     function AnotherDaysData(arrOfDays1, arrOfDays2, objOfCurrentDate,func){
      ...
      for( let i = 0; i <= arrOfDays1.length-1; i++){
        arr.push({day: arrOfDays1[i], date: func(objOfCurrentDate, - (arrOfDays1.length - i))})
        } 
      }
  
      for( let i = 0; i <= arrOfDays2.length-1; i++){
        arr.push({day: arrOfDays2[i], date: func(objOfCurrentDate, + (i + 1))})
       }
       return arr;
    ...
    // console.log(compareDays('чт',findDayOfWeek(currentDay)))
